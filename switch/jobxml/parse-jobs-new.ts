...
interface JobXMLData {
  lineIdentifier: string;
  lineIdentifierNoPrefix?: string;
  jobId: string;
  jobIdNoPrefix?: string;
  jobLineId: string;
  jobName: string;
  recipeId?: string;
  customerId: string;
...
async function uploadToDatabase(jobs: JobXMLData[]) {
    ...
    for (const job of jobs) {
      ...
      if (job.quoteId) {
        try {
          ...
          console.log(`Warning: Error checking quote ${job.quoteId}, using quote ID anyway:`, (error as Error).message);
          validQuoteId = job.quoteId;
        }
      }
